...
class RepoAlunos:
    def __init__(self):
        ...
        with open("D:/ERIC/CURSOS/ESTACIO/ADS/Desenvolvimento Rápido de Aplicação em Python/python/atividade_AV1/nome.txt", 'r') as Nome:
            self.lista_nome = Nome.readlines()
        with open("D:/ERIC/CURSOS/ESTACIO/ADS/Desenvolvimento Rápido de Aplicação em Python/python/atividade_AV1/sobrenome.txt", 'r') as Sobrenome:
    ...
    def gen_Email(self):
        ...
        for i in range(len(self.lista_sobrenome)):
            nome = self.lista_nome[i]
            sobrenome = self.lista_sobrenome[i]
            lista_E.append(f'{nome.strip()}{sobrenome.strip()}@gmail.com'.lower())
        return lista_E
